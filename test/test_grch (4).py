...
def series_comma(text):
    ...
    # df1 = pd.DataFrame(data=data, columns=["Context"])
    lines=[]
    for line in data : 
        # print(line)
        pattern = "\w+, \w+ and"
...
docx_path = os.path.join("C:/Users/ANISH/Desktop/Dev/Internship/", "20102.docx")
# docx_path = os.path.join("C:/Users/ANISH/Desktop/Dev/Internship/Manuscripts", "Test.docx")

text = get_text_from_docx(docx_path)
df = series_comma(text)
...
def grammer_check(docx_path) :
    text = get_text_from_docx(docx_path)
    matches = tool.check(text)
    df = pd.DataFrame(columns=["Message", "Suggestions", "Line", "Context", "Word"])
